...
def create_XFFTSxarray(path_startfile=None, path_antlogfile=None, path_XFFTSdata=None, path_messfiles=None, Tamb=280.0, nchan=32768, tBW=2.5e9):
	...
	xr_data.attrs["v_TOPO"] = v_TOPO*1000.0
	...
	Band_center = float(xr_data.OBS_FREQ) * (1.0 - float(xr_data.VELO)/299792458.0) * (1.0 - xr_data.v_TOPO/299792458.0) - (float(xr_data.OBS_FREQ) - float(xr_data.REST_FREQ)) 
	#Band_start = Band_center - tBW/2.0 * (-1.0)**num_reverse
	#Band_end = Band_center + tBW/2.0 * (-1.0)**num_reverse
	Band_start = Band_center - (tBW/2.0 * (-1.0)**num_reverse) * (1.0 - float(xr_data.VELO)/299792458.0)
	Band_end = Band_center + (tBW/2.0 * (-1.0)**num_reverse) * (1.0 - float(xr_data.VELO)/299792458.0)
